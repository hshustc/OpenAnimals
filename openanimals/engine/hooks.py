...
from openanimals.evaluation.testing import flatten_results_dict
# from openanimals.solver import optim
from openanimals.utils import comm
from openanimals.utils.checkpoint import PeriodicCheckpointer as _PeriodicCheckpointer
from openanimals.utils.events import EventStorage, EventWriter, get_event_storage
from openanimals.utils.file_io import PathManager
from openanimals.utils.precision_bn import update_bn_stats, get_bn_modules
from openanimals.utils.timer import Timer
...
class PeriodicCheckpointer(_PeriodicCheckpointer, HookBase):
    """
    Same as :class:`openanimals.utils.checkpoint.PeriodicCheckpointer`, but as a hook.
...
class AutogradProfiler(HookBase):
    ...
    def after_step(self):
        ...
        if "://" not in out_file:
            ...
            with tempfile.TemporaryDirectory(prefix="openanimals_profiler") as d:
...
class EvalHook(HookBase):
    ...
    def after_train(self):
        ...
        if next_epoch >= self.trainer.max_epoch:
            if not (self._period > 0 and next_epoch % self._period == 0): # avoid duplicate evaluation
                self._do_eval()
