...
from openanimals.data import build_reid_test_loader, build_reid_train_loader
from openanimals.evaluation import (ReidEvaluator,
                                 inference_on_dataset, print_csv_format)
from openanimals.modeling.meta_arch import build_model
from openanimals.solver import build_lr_scheduler, build_optimizer
from openanimals.utils import comm
from openanimals.utils.checkpoint import Checkpointer
from openanimals.utils.collect_env import collect_env_info
from openanimals.utils.env import seed_all_rng
from openanimals.utils.events import CommonMetricPrinter, JSONWriter, TensorboardXWriter
from openanimals.utils.file_io import PathManager
from openanimals.utils.logger import setup_logger
from . import hooks
...
def default_argument_parser():
    """
    Create a parser with some common arguments used by openanimals users.
    Returns:
        argparse.ArgumentParser:
    """
    parser = argparse.ArgumentParser(description="openanimals Training")
...
def default_setup(cfg, args):
    ...
    if hasattr(args, "random_seed") and args.random_seed is not None:
        seed_all_rng(args.random_seed + rank)
    else:
        seed_all_rng()
...
class DefaultTrainer(TrainerBase):
    ...
    It is only guaranteed to work well with the standard models and training workflow in openanimals.
    ...
    def __init__(self, cfg):
        ...
        """
        super().__init__()
        logger = logging.getLogger("openanimals")
        if not logger.isEnabledFor(logging.INFO):  # setup_logger is not called for openanimals
            setup_logger()
    ...
    def build_model(cls, cfg):
        ...
        Returns:
            torch.nn.Module:
        It now calls :func:`openanimals.modeling.build_model`.
    ...
    def build_optimizer(cls, cfg, model):
        ...
        It now calls :func:`openanimals.solver.build_optimizer`.
    ...
    def build_lr_scheduler(cls, cfg, optimizer, iters_per_epoch):
        """
        It now calls :func:`openanimals.solver.build_lr_scheduler`.
    ...
    def build_train_loader(cls, cfg):
        ...
        Returns:
            iterable
        It now calls :func:`openanimals.data.build_reid_train_loader`.
    ...
    def build_test_loader(cls, cfg, dataset_name):
        ...
        Returns:
            iterable
        It now calls :func:`openanimals.data.build_reid_test_loader`.
        Overwrite it if you'd like a different data loader.
        """
