...
from openanimals.config import configurable
...
_root = os.getenv("REID_DATASETS", "datasets")
...
def _train_loader_from_config(cfg, *, train_set=None, transforms=None, sampler=None, **kwargs):
    ...
    if train_set is None:
        ...
        for d in cfg.DATASETS.NAMES:
            if d.startswith('Animals'):
                species = d.split('-')[1] # e.g. Animals-ATRW
                data = DATASET_REGISTRY.get('Animals')(root=_root, species=species, **kwargs)
            else:
...
def _test_loader_from_config(cfg, *, dataset_name=None, test_set=None, num_query=0, transforms=None, **kwargs):
    ...
    if test_set is None:
        ...
        if dataset_name.startswith('Animals'):
            species = dataset_name.split('-')[1] # e.g. Animals-ATRW
            data = DATASET_REGISTRY.get('Animals')(root=_root, species=species, **kwargs)
        else:
